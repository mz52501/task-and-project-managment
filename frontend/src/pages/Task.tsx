...
const Task = () => {
    ...
    const timeEntries = [
        ...
        {id: 4, date: "2024-05-25", hours: "2h 45m", description: "Added user statistics and activity feed"},
        {id: 5, date: "2024-05-24", hours: "1h 30m", description: "Implemented responsive design using Tailwind CSS"},
    ];
    ...
    const comments = [
        {
            ...
            initials: "SM",
            date: "2024-05-28",
            content: "Great progress on the charts! The visualization looks really clean. Make sure to add proper accessibility labels."
        },
        {
            id: 2,
            author: "Alex Brown",
            initials: "AB",
            date: "2024-05-27",
            content: "Can we also include a dark mode toggle in the dashboard? It would be a nice UX enhancement."
        },
        {
            id: 3,
            author: "Mike Johnson",
            initials: "MJ",
            date: "2024-05-26",
            content: "Started working on this task. Planning to have the basic layout done by end of week."
        },
    ];
        ...
        <div className="flex-grow bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto p-6 grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded shadow p-6">
                        <div className="flex justify-between">
                            ...
                            <div className="space-x-2 text-xs">
                                <span
                                    className={`px-2 py-1 rounded ${getColor("priority", task.priority)}`}>{task.priority}</span>
                                <span
                                    className={`px-2 py-1 rounded whitespace-nowrap ${getColor("status", task.status)}`}>{task.status}</span>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 text-sm text-gray-700">
                            <div className="flex items-center gap-2"><User className="w-4 h-4"/>{task.assignee}</div>
                            <div className="flex items-center gap-2"><Calendar className="w-4 h-4"/>{task.dueDate}</div>
                            <div className="flex items-center gap-2"><Clock className="w-4 h-4"/>{task.totalTimeTracked}
                            </div>
                    ...
                    {/* Subtasks */}
                    <div className="bg-white rounded shadow p-6">
                        <h3 className="font-semibold mb-4">Subtasks
                            ({subtasks.filter(s => s.completed).length}/{subtasks.length})</h3>
                        ...
                        <div className="space-y-4">
                            {comments.map(c => (
                                <div key={c.id} className="flex gap-3 text-sm">
                                    ...
                                    <div>
                                        <div className="font-medium">{c.author} <span
                                            className="text-gray-500 text-xs">{c.date}</span></div>
                            ...
                            <div className="flex gap-3 pt-4 border-t">
                                <div
                                    className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold">You
                                </div>
                  ...
                  />
                                    <button
                                        className="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded">Post
                                        Comment
                                    </button>
                                </div>
                            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded shadow p-6 text-sm space-y-3">
                        ...
                        {['Change Status', 'Assign to Someone', 'Set Due Date', 'Change Priority'].map((action) => (
                            <button key={action}
                                    className="w-full border rounded px-3 py-2 hover:bg-gray-50">{action}</button>
                        ))}
                    </div>
                </div>
            </div>
